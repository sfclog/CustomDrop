...
public class ConditionWorlds implements PageMenu {
   ...
   public ConditionWorlds(Player var1, Block var2, Drop var3) {
      this.player = var1;
      this.block = var2;
      this.drop = var3;
   }
   ...
   public Inventory getMenu(int var1) {
      ...
      while(var6.hasNext()) {
         ...
         var8.addAction(new OpenMenu(this.player, new ConditionWorlds(this.player, this.block, this.drop), var1));
         var3.add(var8);
      }

      Inventory var9 = Utilities.BuildPagedInventory(this.player, var3, ChatColor.DARK_GREEN + "[Drop worlds]", 54, var1, (Action)null, false, this, new DropConditions(this.player, this.block, this.drop));
      Utilities.AddCornerInfo(var9, this.block, this.drop);
      return var9;
   }
}
